...
def _resolve_app_root(app_dir: Path) -> Path:
    """
    Tente de normaliser app_dir vers la racine du projet.

    Heuristiques (max 8 niveaux):
    - si app_dir == "src" -> parent
    - remonte tant qu'on ne trouve pas un marqueur projet (pyproject.toml, .git, main.py)
    """
...
def _clip_log_text(s: str, limit: int = 4000) -> str:
    ...
    def _sh(cmd: list[str], cwd: Path | None = None, check: bool = True, quiet: bool = False) -> subprocess.CompletedProcess:
        ...
        # Best effort: log stdout/stderr for debugging or when the command failed.
...
@dataclass(frozen=True)
class Bootstrap:
    """Bootstrap minimal sous forme de classe.

    Objectif :
    - Encapsuler la configuration (app_dir/app_main/app_name)
    - Préparer le terrain pour un futur 'bootstrapd' (superviseur en process séparé)
    - Garder la compatibilité : run_project_cli() reste l’API publique.

    Note :
    - Le run() se termine par un os.execv() via _run_project_cli_impl(),
      donc l'instance n'existe plus après lancement de l'app.
    """

    app_dir: Path
    app_main: Path | None = None
    app_name: str | None = None

    def run(self) -> int:
        return _run_project_cli_impl(app_dir=self.app_dir, app_main=self.app_main, app_name=self.app_name)


def run_project_cli(*, app_dir: Path, app_main: Path | None = None, app_name: str | None = None) -> int:
    """API publique (compat) — wrapper OO."""
    return Bootstrap(app_dir=app_dir, app_main=app_main, app_name=app_name).run()

def _run_project_cli_impl(*, app_dir: Path, app_main: Path | None = None, app_name: str | None = None) -> int:
    """
